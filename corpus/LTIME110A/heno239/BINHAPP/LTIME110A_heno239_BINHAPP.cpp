...
void solve() {
    ...
    rep(i, n) {
        ...
        ori[i] = { max(0,cur),ma,cur-mi,0 };
        orima[i] = val;
    }
    auto f = [&](ar a, ar b) {
        ar res;
        res[0] = a[0] + b[0];
        res[1] = max(a[1] + b[0], a[0] + b[1]);
        res[2] = max(a[2] + b[0], a[0] + b[2]);
        res[3] = max({ a[3] + b[0],a[2] + b[1],a[1] + b[2],a[0] + b[3] });
        return res;
    };
    auto ff = [&](int a, int b) {
        return max(a, b);
    };
    SegT<ar> st(ori, { 0,0 }, f);
    SegT<int> stma(orima, 0, ff);
    //[l,r)
    auto query = [&](int l, int r) {
        ar p = st.query(l, r);
        int v = stma.query(l, r);
        return max(p[3], v);
    };
    rep(i, q) {
        int l, r; cin >> l >> r;
