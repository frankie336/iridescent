...
struct N{
    int v[2][2];
    N(){
        rep(i,2)rep(j,2)v[i][j]=-inf;
        v[0][0]=0;
    }
    N(int pre,int suf,int whole){
        v[0][0]=max(whole,0LL);
        v[1][0]=pre;
        v[0][1]=suf;
        v[1][1]=-inf;
    ...
    static N merge(const N&a,const N&b){
        N res;
        rep(i,2)rep(j,2)rep(x,2-i)rep(y,2-j){
            chmax(res.v[i+x][j+y],a.v[i][j]+b.v[x][y]);
        }
        return res;
    }
};
void slv(){
    ...
    rep(i,n){
        ...
        single[i]=mid;
        rw[i]=N(mx,cur-mn,cur);
    }
    segtree<MaxNode> singleseg(single);
    segtree<N> multseg(rw);
    rep(_,q){
        int l,r;cin>>l>>r;
        l--;
        int ans=singleseg.composite(l,r).v;
        chmax(ans,multseg.composite(l,r).v[1][1]);
        print(ans);
    }
}
signed main(){
