...
P f(P x, P y) {
    chmax(x.se, y.fi);
    if(x.se > x.fi) swap(x.fi, x.se);
    chmax(x.se, y.se);
    if(x.se > x.fi) swap(x.fi, x.se);
    return x;
}
P e() { return pair(pii(-inf<int>, 0), pii(-inf<int>, 0)); }
int main() {
    INT(n, q);
    struct S {
        int sum, best, mi, ma;
    ...
    vl s(n);
    rep(i, n) if(a[i].sum > 0) s[i] = a[i].sum;
    vl ma(n), mi(n);
    rep(i, n) ma[i] = a[i].ma - s[i];
    rep(i, n) mi[i] = (a[i].sum - a[i].mi) - s[i];
    RST<ll> seg(s);
    RMT<ll> rmt(n);
    rep(i, n) rmt.set(i, a[i].best);
    segtree<P, f, e> MA(n), MI(n);
    rep(i, n) {
        MA.set(i, pair(pii(ma[i], i), pii(-inf<int>, 0)));
        MI.set(i, pair(pii(mi[i], i), pii(-inf<int>, 0)));
    }
    rep(q) {
        INT(l, r);
        l--;
        ll res = seg.prod(l, r);
        auto [a, b] = MA.prod(l, r);
        auto [c, d] = MI.prod(l, r);
        ll m = -inf<ll>;
        for(auto i : {a, b})
            for(auto j : {c, d}) {
                if(i.se != j.se) chmax(m, i.fi + j.fi);
            }
        OUT(max(res + m, rmt.prod(l, r)));
    }
}