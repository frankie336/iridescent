...
int main() {
    ...
    while (T--) {
        ...
        for (int j = 0; j < M; j++) {
            int c, b; cin >> c >> b; c--;
            constraints[j] = {c, D / b - 1};
        }
        std::sort(constraints.begin(), constraints.end());
        if (constraints.empty() || constraints.back().first < N-1) {
            constraints.push_back({N-1, D - 1});
        }
        M = int(constraints.size());
        std::vector<num> dp(D);
        dp[D-1] = 1;
        for (int z = M-1; z >= 0; ) {
            int cnt = constraints[z].first;
            int v = constraints[z].second;
            while (z >= 0 && constraints[z].second >= v) {
                z--;
            }
            cnt -= (z == -1 ? -1 : constraints[z].first);
            assert(cnt >= 1);
            while (int(dp.size()) > v+1) {
                num x = dp.back();
                dp.pop_back();
                dp.back() += x;
            }
            assert(int(dp.size()) == v+1);
            std::vector<num> conv(v+1);
            for (int i = 0; i <= v; i++) {
                conv[i] = fact[i+cnt-1] * ifact[cnt-1] * ifact[i];
            }
            std::reverse(conv.begin(), conv.end());
            dp = ecnerwala::fft::fft_mod_multiply(dp, conv);
            dp.erase(dp.begin(), dp.end() - (v+1));
        }
        cout << std::accumulate(dp.begin(), dp.end(), num(0)) << '\n';
    }
    return 0;
}