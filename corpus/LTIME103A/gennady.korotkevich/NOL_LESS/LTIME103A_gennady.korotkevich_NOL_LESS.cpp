...
int main() {
  ...
  while (tt--) {
    ...
    int ptr = 0;
    for (int i = 0; i < (int) a.size(); i++) {
      while (ptr > 0 && -a[i].second <= -a[ptr - 1].second) {
        --ptr;
      }
      a[ptr++] = a[i];
    }
    ...
    vector<Mint> dp(1, 1);
    for (int i = 0; i < (int) a.size(); i++) {
      int cnt = a[i].first - (i == 0 ? 0 : a[i - 1].first);
      vector<Mint> poly(-a[i].second);
      for (int i = 0; i < (int) poly.size(); i++) {
        poly[i] = C(cnt - 1 + i, i);
      }
      auto new_dp = dp * poly;
      new_dp.resize(-a[i].second);
      swap(dp, new_dp);
    }
